...
```{r}
pubg_stats <- read.csv("C:\\Users\\axmac\\Desktop\\git-general-home\\pubg-weapon-stats.csv")
pubg_stats <- pubg_stats %>% select(Weapon.Name:Fire.Mode) %>% mutate(Single = str_detect(Fire.Mode, "Single"), Automatic = str_detect(Fire.Mode, "Automatic"), Burst = str_detect(Fire.Mode, "Burst")) 
...
stk_c <- ps %>% arrange(Shots.to.Kill..Chest.) %>% select(Weapon.Name, Shots.to.Kill..Chest.) %>% head(5)
stk_h <- ps %>% arrange(Shots.to.Kill..Head.) %>% select(Weapon.Name, Shots.to.Kill..Head.) %>% head(5)
